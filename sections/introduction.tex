...
As a result of executing a test $t\in\cT$, \questioner receives a \emph{reply} that reveals $H\in t$ such that $\target\in H$. Upon receiving this reply, the \questioner selects another test $t'\in\cT$ to perform next. The goal is to conceive a strategy of testing, often modeled as a decision tree, which either minimizes the worst-case or average-case number of tests performed until $\target$ is uncovered. We refer to the first problem as \textsc{Worst Case Decision Tree} ($\ProblemPCWCDT$) and to the second as \textsc{Average Case Decision Tree} ($\ProblemPCACDT$).

% We note that the above statement generalizes the classical binary search in (fully or partially) sorted data.
In this work we generalize this process by considering instances with arbitrary precedence constraints between tests given as an arbitrary partial order $(\cT,\preceq)$ on the test set $\cT$.
It is required, that in any valid strategy of testing, a given test $t$ can be performed only after all its predecessors have been performed previously (to which we refer as \emph{precedence-closed} solution). This problem is a natural generalization of the the standard decision tree problems, and is partially motivated by the fact that the decision tree can be interpreted as a schedule of tests. Since task scheduling often involves precedence constraints, it is natural to ask how such constraints affect the complexity of the decision tree problem.
...
% To see a visual example of such a scenario consider Figure~\ref{fig:pcal_example}.
...
To address this issue we use a series of `reductions' between other optimization problems i.~e., using an algorithm for one problem as a subroutine for the other.
In particular, we study set covering with precedence:
...
We ask for a $\cC\subseteq\cS$ that covers at least $f\cdot n$ items from $\cU$ and is \emph{precedence-closed}, that is, for each $x\in\cC$ and each $y\in\cS$ such that $y\preceq x$ it holds $y\in\cC$. We measure the quality of such $\cC$ by either its size $\spr{\cC}$ or the min-sum time it takes to cover an item from $\cU$ (assuming some order of sets in $\cC$ closed under the precedence constraint). Depending on the cost objective we refer to the problem as \textsc{Precedence Constrained Set Cover} ($\ProblemPCSC$) or \textsc{Precedence Constrained Min-Sum Set Cover} ($\ProblemPCMSSC$).
% For a visual example of such a scenario consider Figure~\ref{fig:pccp_example}.
We show how to reduce the \textsc{Optimal Decision Tree} to the above set covering. Then we develop approximation algorithms for the set covering problems themselves. The approximation algorithms that we propose en route, e.g. for precedence constrained set covering, are of independent interest.
