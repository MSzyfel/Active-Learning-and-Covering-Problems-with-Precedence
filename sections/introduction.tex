...
\subsection{Problem formulations} \label{sec:problems}
...
%For any partial order $(X,\preceq)$ we say that $S\subseteq X$ is \emph{precedence-closed} if $x\in S$ implies $y\in S$ for each $y$ such that $y\preceq x$.
Also, let $\closure{S}$ denote the minimal precedence-closed subset of $X$ such that $S\subseteq X$.
...
Given a universum $\cU$ of $n$ items, a family $\cS$ of $m$ subsets of $\cU$, a precedence relation $(\cS,\preceq)$ and $k>0$, find a precedence-closed subfamily $\cC\subseteq\cS$ that covers at least $k$ items and minimizes $\spr{\cC}$.
\end{problem}
...
Given a universum $\cU$ of $n$ items, a family $\cS$ of $m$ subsets of $\cU$, a precedence relation $(\cS,\preceq)$ and $k>0$, find a precedence-closed sequence $(C_1,\ldots,C_l)$ that covers at least $k$ items and minimizes the sum of coverage times of the elements in $C_1\cup\cdots\cup C_l$.
\end{problem}
...
Given a universum $\cU$ of $n$ items, a family $\cG$ of $m$ subsets of $\cU$, a precedence relation $(\cG,\preceq)$, find a precedence-closed subfamily $\cA \subseteq \mathcal{G}$ that maximizes $\Delta\br{\cA}$.
\end{problem}



\begin{problem}[Bounded Max-Density Precedence-Closed Subfamily ($\ProblemBMDPCS$)]
Given a universum $\cU$ of $n$ items, a family $\cG$ of $m$ subsets of $\cU$, a precedence relation $(\cG,\preceq)$ and an integer $B>0$, find a precedence-closed subfamily $\cA \subseteq \mathcal{G}$ that maximizes $\Delta\br{\cA}$ and is size-bounded $\spr{\cA}\leq B$.
\end{problem}
