...
The set $\cH_T\subseteq\cH$ consists of all hypotheses $h$ such that for each test $t\in T$ the reply is a set $U_t\in t$ such that $h\in U_t$.
Note that $\cH_T$ does not depend on the order of performing these tests.
Likewise, if $v$ is any node of a decision tree $D$ then $\cH_v=\cH_T$, where $T=\tests{D}{v}$.
