...
Throughout the paper we use $\cI$ to denote an instance of particular problems, and $\OPT\br{\cI}$ denotes then the cost of an optimal solution on instance $\cI$.
\begin{problem}[Precedence Constrained Worst Case Decision Tree ($\ProblemPCWCDT$)] \label{problem:PCWCDT}
Given a set of hypotheses $\cH$, a set of tests $\cT$ and a precedence relation $(\cT,\preceq)$ on tests, find a precedence-closed decision tree $D$ that and minimizes the worst-case cost $\COSTW\br{D}=\OPT\br{\cI}$.
\end{problem}
\begin{problem}[Precedence Constrained Average Case Decision Tree ($\ProblemPCACDT$)] \label{problem:PCACDT}
Given a set of hypotheses $\cH$, a set of tests $\cT$, a precedence relation $(\cT,\preceq)$ on tests, find a precedence-closed decision tree $D$ that and minimizes the average-case cost $\COSTA\br{D}=\OPT\br{\cI}$.
\end{problem}
...
The \emph{coverage time} of an element $x\in\cov\br{\cC}$ in such precedence-closed sequence $\cC=\br{C_1,\ldots,C_l}$ is the minimum index $i$ such that $x\in C_i$ if such index exists, and $\spr{\cC}$ otherwise.
The symbol $\coverageTime\br{\cC}$ denotes the sum of coverage times of all elements in $\cov\br{\cC}$.

\begin{problem}[Precedence Constrained Set Cover ($\ProblemPCSC$)] \label{problem:PCSC}
Given a universum $\cU$ of $n$ items, a family $\cS$ of $m$ subsets of $\cU$, a precedence relation $(\cS,\preceq)$ and $0<f<1$, find a precedence-closed subfamily $\cC\subseteq\cS$ that covers at least $f\cdot n$ items and minimizes $\spr{\cC}$.
\end{problem}
\begin{problem}[Precedence Constrained Min-Sum Set Cover ($\ProblemPCMSSC$)] \label{problem:PCMSSC}
Given a universum $\cU$ of $n$ items, a family $\cS$ of $m$ subsets of $\cU$, a precedence relation $(\cS,\preceq)$ and $0<f<1$, find a precedence-closed sequence $\cC$ that covers at least $f\cdot n$ items and minimizes $\coverageTime\br{\cC}$.
\end{problem}
\begin{problem}[Budgeted Precedence Constrained Set Cover ($\ProblemBPCSC$)] \label{problem:BPCSC}
...
We say that an algorithm is $(\gamma,\alpha)$-approximation for $\ProblemPCSC$, where $\gamma\geq 1$ and $\alpha\geq 1$, if for any input instance $\cI=(\cU,\cS,\preceq,f)$ it returns a solution $\cC$ such that $\cov\br{\cC}\geq f\cdot n/\alpha$ and $\spr{\cC}\leq\gamma\cdot\optPCSC{\cI}$.
From a $(\gamma,\alpha)$-approximation $\cC$ for $\ProblemPCMSSC$ we require $\cov\br{\cC}\geq f\cdot n/\alpha$ and $\coverageTime\br{\cC}\leq\gamma\cdot\OPT\br{\cI}$.
...
The \emph{density} of a nonempty subfamily $\cA$ on a universum $X$ is
$
\Delta\br{\cA, X} = \frac{\spr{\cov\br{\cA, X}}}{\spr{\cA}}.
$
We write $\Delta\br{\cA}$ when $X$ there is no ambiguity.

\begin{problem}[Max-Density Precedence-Closed Subfamily ($\ProblemMDPCS$)] \label{problem:MDPCS}
...
\begin{problem}[Bounded Max-Density Precedence-Closed Subfamily ($\ProblemBMDPCS$)] \label{problem:BMDPCS}
...
An \emph{$r$-tour} $\tau$ is a walk that starts and ends at vertex $r \in V$. Denote by $\spr{\tau}$ the length of tour $\tau$ with respect to $d$.
...
\begin{problem}[Group Steiner Tree (GST)] \label{problem:GST}
...
\begin{problem}[Group Steiner Orienteering (GSO)] \label{problem:GSO}
...
\end{problem}
\begin{problem}[Partial Latency Group Steiner Tree (LPGST)] \label{problem:LPGST}
