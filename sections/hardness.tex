...
We begin by showing that the decision tree problem is NP-hard even for the very special of binary searching with precedence.
...
\begin{theorem}\label{thm:PCWCDT-NP-hard}
    Worst-Case Decision Tree With Precedence Constraints is \NPhard.
\end{theorem}
\begin{proof}
	...
	The idea is to:
	\begin{itemize}
		\item Construct tests corresponding, separately, to  variables and clauses. 
		...
		\item All the variable-test (for hypotheses corresponding to variables) can be performed at the same time.
		Hence, either the variable-test or test corresponding to the negation of the variable can be performed [chosen] in a given turn.
		And, correspondingly, the tests corresponding to the negation (or the variable) can be perform in the next turn.
		\item Construct literal-tests [tests per literal]. Make every literal-test dependent on the corresponding variable.
		\item Now, if at least one variable-test was performed in the time $t$, then at least one literal-test can be performed in $t+1$.
	...
	\begin{enumerate}
		...
		\]
		Denote the tests between the consecutive pairs of hypotheses as: $\litedge{i, 0}$, $\litedge{i, v_i^1}$, $\litedge{i, v_i^2}$, $\litedge{i, v_i^3}$, $\litedge{i, 0'}$Binary Search.
		...
		Connect $\coord{c}{|C|}{4}$ to $\coord{v}{1}{1}$ by $\sepedge{|C|}$, $|C|$-th separator test.
		Continue with variable gadgets in the same way; hence the total number of separator tests is $|C| + |V| - 1$.
	\end{enumerate}
	To define the order on the edges:
	\begin{enumerate}
		\item Observe that there are exactly $1+2+\ldots+2^{k}$ separator tests, for some $k > 0$ due to introduction of dummy variables.
		...
		\item For every cBinary Searchlause $c_i$ and every $v \in \{v_i^1,v_i^2,v_i^3\}$, make $\litedge{i, v}$ dependent on $\varedge{v^+}$ if $v$ is positive literal; make it dependent on $\varedge{v^-}$ otherwise.
	...
	i.e. the problem to determine if for a given input $(P, O, t)$ there exists a strategy of worst case number of tests less than or equal to $t$.
	
	To prove that the problem is in \NP we observe that given decision tree certifies maximum number of tests.
	...
	\begin{itemize}
		\item Perform the separator tests in time up to $k$.
		\item For every variable $v$, if variable was assigned $T$, perform $\varedge{v^+}$ it in time $k+1$; otherwise perform $\varedge{v^-}$.
		      Hence, they are performed in times $k+1$ and $k+2$, respectively. 
	...
	\begin{itemize}
		...
		\item The earliest possible time to finish separator tests is $k$.
		\item Due to this we have to perform at least one test in time $k+2$ for every clause-gadget $c_i$.
		Due to the fact that $\litedge{i, 0}$ and $\litedge{i, 0'}$ cannot be performed before $k+3$, it has to be the case that one of $\litedge{i, v_i^1}$, $\litedge{i, v_i^2}$, $\litedge{i, v_i^3}$ was queried.
		Hence, for at least one of these tests preconditions were met [test was performed] in time $k+1$.
		The variable-tests queried in $k+1$ to a valuation of the variables matching all the clauses.
	\end{itemize}
...
\begin{theorem}
    Average-Case Decision Tree With Precedence Constraints is \NPhard.
\end{theorem}
\begin{proof}
	...
	Hence:
	\begin{itemize}
	\item In any reasonable solution the separator-tests will be performed in times $1, 2, \ldots, k$.
	\item 	Now, the $|V|$ variable-tests correspond to total quarry time $|V| (3k+5)$, denoted by $T_{var}$, because 1 hypothesis is be decided in time $k+1$ and $2$ in $k+2$.
	...
	Hence this gives total number of tests spent on these hypotheses is $|C|(6k + 22)$, denoted by $T_{clauses}$.  
	\end{itemize} 
...
\begin{proof}
    ...
    Therefore, any valid decision tree for the constructed instance of PCWCDT corresponds to a valid selection of sets in PCSC and vice versa. Thus, since the size of the reduction is linear any $\alpha$-approximation for PCWCDT would yield an $\cO\br{\alpha}$-approximation for PCSC.
\end{proof}
...
\begin{proof}
    ...
    \TP{A to $\mathbb{E}[\spr{E_{uv}}] = \mathbb{E}[\spr{U_u \cup U_v}] = np^2$ jest dobrze? Raczje powinna być ta część wspólna.}
    ...
    \MS{Tutaj dowod jest jeszcze enigmatyczny dla mnie dlatego zostawiam to na razie. T	\TP{Podkreślić mocno, że skopiowane to }ODO: pokazac, ze jak nie ma komponentu to koszt rozwiazania mocno rosnie}.
    \begin{enumerate}
    	\item Consider any solution consisting of $x$ vertex sets taken with $\lambda$ multiplicity [hence there are $x\lambda$ sets].
    	The expected number of edges on this vertices is $E_{\#e} \le \frac{x^2}{\sqrt{N}}$.
    	There are $\binom{N}{x} \le N^x$ such sets.
    	\item By Chernoff Bounds the probability that a given set of $x$ vertices has more than $3/2 E_{\#e}$ edges is at most $\exp(-E_{\#e}/12)$.
    	Hence the probability that any set of vertices has more that $3/2 E_{\#e}$ edges is at most $N^{x}\exp(-E_{\#e}/12)$.
    	\item Due to the fact that $\alpha = \frac{1}{2}$,
    	$E_{\#e}=\frac{x^2}{2\sqrt{N}}$.
    	Hence if one assumes $x \ge C\sqrt{N}\log N$, one gets $E_{\#e} \ge \frac{C}{2}\sqrt{N}\log^2N$.
    	Due to this the probability that any set of vertices has more that $3/2 E_{\#e}$ edges is at most
    	\TP{Praca trwa...}
    \end{enumerate}
