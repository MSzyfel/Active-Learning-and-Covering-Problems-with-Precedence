...
\begin{proof}
    ...
    $\ProblemGST$ on trees cannot be approximated within a factor of $\cO\br{\log^2 n}$ unless $\text{NP}\subseteq \text{ZTIME}\br{n^{\text{polylog}(n)}}$ \cite{PolylogarithmicInapproximability}. In the latter reduction, all distances are of form $2^{-h}$ with exponent ranging from $0$ to $h=\cO\br{\log^{1-\epsilon} n}$. Therefore, we can scale all of them by $2^h=\text{poly}\br{n}$ to obtain integer weights without affecting the approximation ratio.
