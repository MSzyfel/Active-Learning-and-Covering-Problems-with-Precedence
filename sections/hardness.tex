...
\end{theorem}
\begin{proof}
    Let $(\cU, \cS, \preceq)$ be an instance of $\ProblemPCSC$ where $\cU$ is the universum of elements, $\cS$ is the family of sets and $\preceq$ are the precedence constraints on $\cS$. We construct an instance of $\ProblemPCWCDT$ as follows. For each element $u \in \cU$, we create two representative hypotheses $h_u^0$ and $h_u^1$. 
...
\begin{proof}
    ...
    Next, we define the universum of elements to be covered. Let $\cU=\{0,1,\dots, n\}$ for some appropriately chosen natural number $n$ to be determined later. Every vertex representative set contains only the special element $0$, that is, $V_{v, i}=\{0\}$ for all $v \in [N]$ and $i \in [\lambda]$.
