...
% \end{itemize}
Using this conjecture one can show the following inapproximability results for $\ProblemPCMSSC$ \cite{PCMSSC}: \DD{tutaj jest nieco niejasne, czy można pokazać czy oni pokazali? W sumie nie jest dla mnie jasne dlaczego sa dwie funkcje podane skoro mówimy o tym samym problemie: gdyż jeśli nie potrafimy aproksymować z ratio $\cO\br{m^{1/6-\epsilon}}$ to z tym dokładniejszym tym bardziej (?)}
