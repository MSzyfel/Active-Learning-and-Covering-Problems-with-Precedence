...
\begin{proof}
    ...
    With this choice, we have $\mathbb{E}[\spr{\{E_{uv}\colon j\in E_{uv}, uv \in E(\mathcal{P})\}}] \geq 64\log^2 N$, which will allow us to apply concentration bounds effectively.
    ...
    Therefore, with high probability, after selecting $\lambda x$ vertex representatives and at most $x^2/\sqrt{N}$ edge representatives, the total number of elements from $\{1,\dots, n\}$ that are covered is at most:
    $$
    \frac{x^2}{\sqrt{N}} \cdot 2np^2 = 2np^2 \cdot \frac{x^2}{\sqrt{N}}.
    $$
    
    Substituting $p^2 = 1024 \cdot N^{-3/4+\gamma/2}\cdot \log^2 N$, we get:
    $$
    2048 \cdot n \cdot N^{-3/4+\gamma/2}\cdot \log^2 N \cdot \frac{x^2}{\sqrt{N}} = 2048 \cdot n \cdot N^{-5/4+\gamma/2}\cdot \log^2 N \cdot x^2.
    $$
    
    Now, suppose $x \leq N^{(5-2\gamma)/8}$. Then:
    $$
    2048 \cdot n \cdot N^{-5/4+\gamma/2}\cdot \log^2 N \cdot x^2 \leq 2048 \cdot n \cdot N^{-5/4+\gamma/2}\cdot \log^2 N \cdot N^{(5-2\gamma)/4} = 2048 \cdot n \cdot \log^2 N.
    $$
    ...
    We now compare the costs in the two cases to establish the inapproximability ratio. When a planted component exists, with high probability there is a solution of cost at most:
    $$
    \lambda\sqrt{N} + N^{3/4-\gamma/2}.
    $$
    
    When no planted component exists, with high probability any solution has cost at least:
    $$
    \lambda N^{(5-2\gamma)/8} + N^{(5-2\gamma)/4}.
    $$
    
    The ratio between these costs is:
    $$
    \frac{\lambda N^{(5-2\gamma)/8} + N^{(5-2\gamma)/4}}{\lambda\sqrt{N} + N^{3/4-\gamma/2}}.
    $$
    
    By choosing $\lambda = N^{1/4}$ (or any $\lambda \geq N^{1/4}$) and letting $\gamma$ approach $0$, the dominant terms in the numerator and denominator become $N^{5/8}$ and $N^{1/2}$ respectively, giving a ratio approaching:
    $$
    \frac{N^{5/8}}{N^{1/2}} = N^{1/8}.
    $$
    
    Since we chose $n = \Theta(N^{3/4})$, we have that the gap is $\Theta(N^{1/8}) = \Theta((N^{3/4})^{1/6}) = \Theta(n^{1/6})$. For any $\epsilon > 0$, by choosing $\gamma$ sufficiently small (but positive), we can make this gap arbitrarily close to $n^{1/6-\epsilon}$.
    
    Furthermore, the total number of sets $m$ in the constructed instance is $m = \lambda N + |\mathcal{E}|$. For a graph drawn from $G(N, N^{-1/2})$ (possibly with a planted component), the number of edges is highly concentrated around $\binom{N}{2} \cdot N^{-1/2} = \Theta(N^{3/2})$. With $\lambda = N^{1/4}$, we have $m = \Theta(N^{3/2})$. Therefore, $n = \Theta(N^{3/4}) = \Theta(m^{1/2})$, and the gap can also be expressed as:
    $$
    \Theta(N^{1/8}) = \Theta((N^{3/2})^{1/12}) = \Theta(m^{1/12}).
    $$
    
    Thus, any polynomial-time algorithm for \ProblemPCSC\ with approximation factor better than $\cO(n^{1/6-\epsilon})$ or $\cO(m^{1/12-\epsilon})$ for any $\epsilon > 0$ could distinguish between the two cases of the Planted Dense Subgraph problem with significant advantage, contradicting the Planted Dense Subgraph Conjecture. This completes the proof.
\end{proof}
