...
The only difference is a black-box procedure that is used to solve a particular subproblem -- either $\ProblemPCSC$ or $\ProblemPCMSSC$. Note that when we are dealing with $\ProblemPCACDT$, the input instance might also include probabilities which we also pass to the black-box.
...
We will use sepcovers to lowerbound optimal solution:
...
\end{theorem}
\begin{proof}
     Consider a call to \ProcDecisionTree on input $\cI=\br{\cH, \cT, \cF, \blackBox}$, where $\blackBox$ is a $\br{\alpha,\gamma}$-approximation algorithm for $\ProblemPCSC$.
     ...
     Assume by induction that for every $\cI' = \br{\cH', \cT, \cF}$ such that $\cH' \subseteq \cH\setminus S$ and $n' = \spr{\cH'}$ we have $\COSTW\br{D'} \leq \frac{\alpha}{\log\br{\frac{4\gamma}{4\gamma -1}}}\cdot \log n' \cdot \OPT\br{\cI'}$, where $D'$ is the decision tree returned by \ProcDecisionTree on input $\cI'$.
...
%             \leq & \alpha \cdot \spr{S^*} + \max_{\cH' \in \cH \setminus S} \frac{\alpha}{\log\br{\frac{4\gamma}{4\gamma -1}}}\cdot \log n' \cdot \OPT\br{\cI'} \\
...
%        \end{align*}
        \begin{align*}
            ...
            \leq & \alpha \cdot \spr{\sepcover{\cI}} + \max_{\cH' \in \cH \setminus S} \frac{\alpha}{\log\br{\frac{4\gamma}{4\gamma -1}}}\cdot \log n' \cdot \OPT\br{\cI'} \\
...
This allows to use $\COSTA\br{\sepcover{\cI}}$ as a lower bound on $\OPTA\br{\cI}$ in the analysis of the approximation algorithm for $\ProblemPCACDT$.
...
For an instance $\cI=\br{\cH, \cT, \cF, p}$, define $p'(h)=z(h)/s_z$ for each $h\in\cH$, where  $z(h) = \cl{p(h)/q}$ and $s_z=\sum_{h\in\cH}z(h)$, $q=\frac{1}{nm}$, $n=\spr{\cH}$ and $m=\spr{\cT}$.
We say that $p'$ is a \emph{$q$-rounding} of $p$.
Note that $p'\br{\cH}=1$ and thus $p'$ is indeed a probability distribution over $\cH$.
\DD{dodalem lower bound dla lemma}
\begin{lemma} \label{lemma:rounding}
    Let $\cI=\br{\cH, \cT, \cF, p}$ be any instance of $\ProblemPCACDT$ and let $p'$ be the $q$-rounding of $p$.
    Then, for the instance $\cI'=\br{\cH, \cT, \cF, p'}$ it holds $z(h)\in \mathbb{N}$ and $\OPTA\br{\cI}\leq\OPTA\br{\cI'} \leq 2 \cdot \OPTA\br{\cI}$.
\end{lemma}
\begin{proof}
    Note that the lower bound $\OPTA\br{\cI}\leq\OPTA\br{\cI'}$ follows from $p\leq p'$, and it remains to upper bound $\OPTA\br{\cI'}$.
    ...
    \end{align*}
    where the second inequality is by Observation~\ref{obs:dec-tree-averaging} again and $\spr{\cH_v}\leq n$ for each node $v$, the third inequality is by $s_z\geq nm$ and the last inequality follows since $\COSTA\br{D, \cI} \geq p\br{\cH} = 1$.
...
Therefore, from now on we will assume that the input instance $\cI=\br{\cH, \cT, \cF, p}$ of $\ProblemPCACDT$ is such that for each $h\in\cH$, $p(h)=i/j$ for integers $i<j=\cO(nm)$, without impacting the asymptotic approximation ratio of our algorithm. 
...
\begin{lemma}\label{lemma:cover_sep_average_case}
    Let $\cI=\br{\cH, \cT, \cF}$ be any uniform distribution $\ProblemPCACDT$ instance. Let $S^*$ be an optimal solution to $\ProblemPCMSSC$ on instance $\br{\cH, \xi(\cT), \preceq_{\xi}, f}$ with $f=1/4$.
...
\begin{theorem}\label{thm:PCMSSC-to-PCACDT}
    If there is an $\br{\beta, \mu}$-bicriteria approximation algorithm for $\ProblemPCMSSC$, then there is a $\cO\br{\frac{\beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log \br{m+n}}$-approximation algorithm for $\ProblemPCACDT$. In particular when $\mu = \cO\br{1}$, the approximation is $\cO\br{\beta \cdot \log \br{m+n}}$.
\end{theorem}
\begin{proof}
    Let $\cI=\br{\cH, \cT, \cF, p}$ be an instance of $\ProblemPCACDT$ and let $p'$ be the $q$-rounding of $p$ with $q=\frac{1}{nm}$.
    By Lemma~\ref{lemma:rounding}, for the rounded instance $\cI'=\br{\cH, \cT, \cF, p'}$ we have $\OPTA\br{\cI}\leq\OPTA\br{\cI'} \leq 2 \cdot \OPTA\br{\cI}$ and $z(h)\in \mathbb{N}$ for each $h\in\cH$, where $z(h) = \cl{p(h)/q}$ and $s_z=\sum_{h\in\cH}z(h)$.
    
    Consider a call to \ProcDecisionTree on input $\cI'=\br{\cH, \cT, \cF, p', \blackBox}$, where $\blackBox$ is a $\br{\beta,\mu}$-approximation algorithm for $\ProblemPCMSSC$.
    Let $D$ be the decision tree returned by this call.
    We will prove by induction on $s_z$ that
    \begin{equation} \label{eq:ACinduction-sz}
     \COSTA\br{D, \cI'} \leq \frac{\beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log s_z \cdot \OPTA\br{\cI'}.
    \end{equation}

    The base case when $s_z=1$ (equivalently, $\spr{\cH}=1$) is trivial since the cost of the decision tree is 0.
    Assume by induction that for every $\cI'' = \br{\cH', \cT, \cF, p'}$ such that $\cH' \subseteq \cH\setminus S$ and $s_z' = \sum_{h\in\cH'}z(h)$ we have $\COSTA\br{D'} \leq \frac{\beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log s_z' \cdot \OPTA\br{\cI''}$, where $D'$ is the decision tree returned by \ProcDecisionTree on input $\cI''$.
    We have
\begin{align*}
    \COSTA\br{D, \cI'} \leq & \COSTA\br{D_S, \cI'} + \sum_{\cH' \in \cH \setminus S} \COSTA\br{D'(\cH'), \cI''} \\
    \leq & \beta \cdot \COSTA\br{\sepcover{\cI'}, \cI'} + \sum_{\cH' \in \cH \setminus S} \frac{\beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log s_z' \cdot \OPTA\br{\cI''} \\
    \leq & \beta \cdot \OPTA\br{\cI'} + \frac{\beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log \br{\frac{\br{4\mu-1}\cdot s_z}{4\mu}} \cdot \OPTA\br{\cI'} \\
    = & \beta \cdot \OPTA\br{\cI'} + \frac{\beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log s_z \cdot \OPTA\br{\cI'} - \beta\cdot \OPTA\br{\cI'} \\
    = & \frac{\beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log s_z \cdot \OPTA\br{\cI'},
\end{align*}
where the second inequality follows by the induction hypothesis and Observation~\ref{obs:DS-average}, the third inequality follows by Observation~\ref{obs:sepcoverAC}, Lemma~\ref{lemma:subspace_opt} and the fact that $s_z'\leq \frac{(4\mu -1)}{4\mu}\cdot s_z$ for every $\cH' \in \cH\setminus S$ (since the $\blackBox$ covers a $1/(4\mu)$ fraction of the total probability mass).

Since $s_z \leq nm + n$ and by Lemma~\ref{lemma:rounding}, we have
\begin{align*}
\COSTA\br{D, \cI} &\leq \COSTA\br{D, \cI'} \leq \frac{\beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log \br{nm+n} \cdot \OPTA\br{\cI'} 
\\&\leq 
\frac{2\cdot\beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log \br{nm+n} \cdot \OPTA\br{\cI}\leq \frac{4\cdot \beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log \br{m+n} \cdot \OPTA\br{\cI}
\end{align*}
