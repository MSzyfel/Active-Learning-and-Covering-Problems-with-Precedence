...
\begin{observation} \label{obs:sepcover}
...
It uses a $\br{\gamma,\alpha}$-approximation algorithm for $\ProblemPCSC$ as a black-box subroutine, in order to find a set cover $S$ for  $\br{\cH, \xi(\cT), \preceq_{\xi}, \spr{\cH}/4}$.
    ...
    Note that the $S$ is supposed to play the role of $\sepcover{\cI}$ according to the bound in Lemma~\ref{lemma:cover_sep_worst_case} and hence we indeed can afford constructing an arbitrary decision tree on the tests in $S$.
...
%     For each $\cH' \in \cH-S$, we recursively call \ProcWorstDecisionTree on instance $\br{\cH', \cT-S, \cF-S}$ and attach the returned decision tree to the leaf of $D_S$ corresponding to $\cH'$. Finally, we return the constructed decision tree $D$.
...
The following observation is due to Lemmas \ref{lemma:subspace_opt} and \ref{lemma:cover_sep_worst_case}.:
    \begin{observation} \label{obs:DS}
        Consider $S$ and $D_S$ computed in Algorithm~\ref{alg:worstDecisionTree}.
...
\end{theorem}
\begin{proof}
     Let $D$ be the decision tree returned by \ProcWorstDecisionTree on input $\cI=\br{\cH, \cT, \cF}$.
...
%        \end{align*}
        \begin{align*}
            ...
            = & \frac{\alpha}{\log\br{\frac{4\gamma}{4\gamma -1}}}\cdot \log n \cdot \OPTW\br{\cI},
        \end{align*}
        where the second inequality follows by the induction hypothesis and Observation~\ref{obs:DS}, the third inequality follows by Observation~\ref{obs:sepcover}, Lemma \ref{lemma:subspace_opt} and the fact that $n'=\spr{\cH'} \leq \frac{(4\gamma -1)}{4\gamma}\cdot n$ for every $\cH' \in \cH\setminus S$.
...
We follow a similar idea, however we use the connection to PCMSSC instead of $\ProblemPCSC$.
...
\end{theorem}
\begin{proof}
The idea behind Algorithm \ref{alg:averageDecisionTree} is the same as for the worst case version of the problem except the fact that we use a solution to PCMSSC instead of $\ProblemPCSC$.
