...
\begin{definition}[Sepcover]
  ...
  If $\COSTW\br{D}=\OPT\br{\cI}$, then we write $\sepcover{\cI} = \sepcover{D}$ (ties broken arbitrarily).
\end{definition}
...
$
\xi(t)=\brc{h\in\cH \mid h\in H \textup{ for some }H\in t\textup{ s.t. }  \spr{H}\leq\frac{3}{4}\spr{\cH}}.
$
...
\begin{lemma}
    We have $\spr{\cU}\geq 3\cdot\spr{\cH}/4$.
\end{lemma}
\begin{proof}
    If $\spr{\cH}=4$, then any test covers all $\cH$ and the claim holds trivially. Assume that $\spr{\cH}>4$. We actually show, that there is at most one hypothesis that is not in $\cU$. To see this observe that if a hypothesis $h_1$ is uncovered, then for every test $t\in\cT$, it holds that for the response $H\in t$ containing $h_1$, $\spr{H}>\frac{3}{4}\spr{\cH}$. Assume that there is a second hypothesis $h_2$ that is also uncovered. By definition we know that there exists a test which distinguishes $h_1$ and $h_2$ so $h_2$ needs to belong to a response of size at most $\spr{\cH}/4$, a contradiction.
\end{proof}
...
Let $\cC$ be the cover obtained by the black-box procedure on the corresponding instance. Assume that $\cC$ contains some ordering of its elements. If $\cC$ is the output of an approximation algorithm for $\ProblemPCSC$, then the ordering is a topological ordering of the tests in $\cC$ according to the precedence constraints.
...
We iteratively build a decision tree $D$ by appending tests associated with consecutive sets from $\cC$. During each such iteration, we look at all possible responses to the appended test and for each sufficiently small response $\cH'$ ($\spr{\cH'} \leq \frac{16\beta-3}{16\beta}\cdot \spr{\cH}$), we recursively call \ProcDecisionTree on the subinstance induced by $\cH'$. If there exists a large response $\cH'$ ($\spr{\cH'} > \frac{16\beta-3}{16\beta}\cdot \spr{\cH}$), then we do not make a recursive call for it and simply continue appending tests from $\cC$ as a response associated with $\cH'$. Note that since $\frac{16\beta-3}{16\beta}>\frac{1}{2}$ for any $\beta \geq 1$, there is at most one such response. 
...
Let $D_{\cC}$ be a decision tree restricted only to tests from $\cC$. The following observation is due to Lemmas \ref{lemma:subspace_opt} and \ref{lemma:cover_sep_worst_case}.:
    \begin{observation} \label{obs:DS}
        Consider $D_{\cC}$ computed in Algorithm~\ref{alg:worstDecisionTree}, that uses an $\br{\gamma,\alpha}$-approximation algorithm for $\ProblemPCSC$ as the $\blackBox$ subroutine.
...
$\cO\br{\frac{\alpha}{\log\br{\frac{16\beta}{16\beta -3}}} \cdot \log n}$-approximation algorithm for $\ProblemPCWCDT$. 
...
\begin{proof}
     ...
     \COSTW\br{D} \leq \frac{\alpha}{\log\br{\frac{16\beta}{16\beta -3}}}\cdot \log n \cdot \OPT\br{\cI}.
     \end{equation}

     The proof is by induction on $n$. The base case when $n=1$ is trivial since the the cost of the decision tree is 0.
     Assume by induction that for every $\cI' = \br{\cH', \cT, \preceq}$ such that $\spr{\cH'}<\spr{\cH}$ and $n' = \spr{\cH'}$ we have $\COSTW\br{D'} \leq \frac{\alpha}{\log\br{\frac{16\beta}{16\beta -3}}}\cdot \log n' \cdot \OPT\br{\cI'}$, where $D'$ is the decision tree recursively returned by \ProcDecisionTree on input $\cI'$.
...
%        \end{align*}
        \begin{align*}
            ...
            \leq & \alpha \cdot \spr{\sepcover{\cI}} + \max_{\cH'(D')} \frac{\alpha}{\log\br{\frac{16\beta}{16\beta -3}}}\cdot \log n' \cdot \OPT\br{\cI'} \\
            \leq & \alpha \cdot \OPT\br{\cI} + \frac{\alpha}{\log\br{\frac{16\beta}{16\beta -3}}}\cdot \log \br{\frac{\br{{16\beta-3}}\cdot n}{16\beta}} \cdot \OPT\br{\cI} \\
            = & \alpha \cdot \OPT\br{\cI} + \frac{\alpha}{\log\br{\frac{16\beta}{16\beta -3}}}\cdot \log n \cdot \OPT\br{\cI} - \alpha\cdot \OPT\br{\cI} \\
            = & \frac{\alpha}{\log\br{\frac{16\beta}{16\beta -3}}}\cdot \log n \cdot \OPT\br{\cI},
        \end{align*}
        where the second inequality follows by the induction hypothesis and Observation~\ref{obs:DS}, the third inequality follows by Observation~\ref{obs:sepcoverWC}, Lemma \ref{lemma:subspace_opt} and the fact that $n'=\spr{\cH'} \leq \br{1-\frac{3}{4}\cdot\frac{1}{4\cdot\beta}}\cdot n=\frac{(16\beta -3)}{16\beta}\cdot n$ for every response $\cH'$ associated with a test outside of $D_{\cC}$.
...
\begin{observation} \label{obs:WCdecomposition}
    ...
    Then,
    \[
    \COSTA\br{D} = \COSTA\br{D_{\cC}} + \sum_{D'\in D\setminus V\br{D_{\cC}}} \COSTA\br{D'}.
    \]
%     $$
...
\begin{theorem}\label{thm:PCMSSC-to-PCACDT}
    If there is an $\br{\alpha, \beta}$-bicriteria approximation algorithm for $\ProblemPCMSSC$, then there is a $\cO\br{\frac{\alpha}{\log\br{\frac{16\beta}{16\beta -3}}}\cdot \log n}$-approximation algorithm for $\ProblemPCACDT$. 
...
\begin{proof}
    ...
    \begin{equation} \label{eq:D-average}
     \COSTA\br{D} \leq \frac{\alpha}{\log\br{\frac{16\beta}{16\beta -3}}}\cdot \log n \cdot \OPT\br{\cI}.
    \end{equation}

    The proof is by induction on $n$. The base case when $n=1$ is trivial since the cost of the decision tree is 0.
    Assume by induction that for every $\cI' = \br{\cH', \cT, \preceq}$ such that $\spr{\cH'}<\spr{\cH}$ and $n' = \spr{\cH'}$ we have $\COSTA\br{D'} \leq \frac{\alpha}{\log\br{\frac{16\beta}{16\beta -3}}}\cdot \log n' \cdot \OPT\br{\cI'}$, where $D'$ is the decision tree recursively returned by \ProcDecisionTree on input $\cI'$.
    To point out the dependency of $D'$ on $\cH'$, we write $D'(\cH')$.
    We have
\begin{align*}
    \COSTA\br{D} \leq & \COSTA\br{D_{\cC}} + \sum_{\cH'(D')} \COSTA\br{D'(\cH')} \\
    \leq & \alpha \cdot \COSTA\br{\sepcover{\cI}} + \sum_{\cH'(D')} \frac{\alpha}{\log\br{\frac{16\beta}{16\beta -3}}}\cdot \log n' \cdot \OPT\br{\cI'} \\
    \leq & \alpha \cdot \OPT\br{\cI} + \frac{\alpha}{\log\br{\frac{16\beta}{16\beta -3}}}\cdot \log \br{\frac{\br{{16\beta-3}}\cdot n}{16\beta}} \cdot \OPT\br{\cI} \\
    = & \alpha \cdot \OPT\br{\cI} + \frac{\alpha}{\log\br{\frac{16\beta}{16\beta -3}}}\cdot \log n \cdot \OPT\br{\cI} - \alpha\cdot \OPT\br{\cI} \\
    = & \frac{\alpha}{\log\br{\frac{16\beta}{16\beta -3}}}\cdot \log n \cdot \OPT\br{\cI},
\end{align*}
