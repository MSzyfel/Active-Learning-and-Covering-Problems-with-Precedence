...
\end{observation}
\begin{proof}
    Assume that $\cH\setminus\cU$ is not empty, otherwise we actually have $\COSTA\br{D_{\cC}} \leq \alpha \cdot \COSTA\br{\sepcover{\cI_{\cU}}}$. Let $h\in \cH\setminus\cU$. Trivially, we have that the contribution of $h$ to $\COSTA\br{D_{\cC}}$ is at most $\spr{\cC}$. We also have that $\spr{\cC}\leq \coverageTime\br{\cC}$. Therefore we have that $\COSTA\br{D_{\cC}} \leq \alpha \cdot \COSTA\br{\sepcover{\cI_{\cU}}} + \spr{\cC} \leq 2\alpha \cdot \COSTA\br{\sepcover{\cI_{\cU}}}$.
\end{proof}
...
\begin{proof}
    ...
    Assume by induction that for every $\cI' = \br{\cH', \cT, \preceq}$ such that $\spr{\cH'}<\spr{\cH}$ and $n' = \spr{\cH'}$ we have $\COSTA\br{D'} \leq \frac{2\alpha}{\log\br{\frac{5\beta}{5\beta -1}}}\cdot \log n' \cdot \OPT\br{\cI'}$, where $D'$ is the decision tree recursively returned by \ProcDecisionTree on input $\cI'$.
    To point out the dependency of $D'$ on $\cH'$, we write $D'(\cH')$.
    We have
\begin{align*}
