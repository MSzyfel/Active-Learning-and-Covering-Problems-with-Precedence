...
\begin{definition}[Coversep]
    Let $D$ be any decision tree for $I=\br{\mathcal{H}, \mathcal{T}, \mathcal{F}}$. We define a sequence of tests $P_D$ called \emph{coversep} as follows. Initially, $P_D$ is empty and $\mathcal{H}' = \mathcal{H}$. While $\spr{\mathcal{H}'}> \spr{\mathcal{H}}/2$, we append to $P_D$ the test $r\br{D_{\mathcal{H}'}}$ and update $\mathcal{H}'$ to be the set of hypotheses corresponding to the child of $D_{\mathcal{H}'}$ that contains the most hypotheses. If $\COSTW\br{D}=\OPTW\br{I}$, then we denote $P^*\br{I} = P_D$ (ties broken arbitrarily).
\end{definition}
