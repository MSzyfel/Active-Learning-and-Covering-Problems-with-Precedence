...
It should be noted that $\spr{\cU}\geq \spr{\cH}-1$. This is because there is at most one hypothesis that is not covered by any test in $\cT$. To see this observe that if a hypothesis $h_1$ is uncovered, then for every test $t\in\cT$, it holds that for the response $H\in t$ containing $h_1$, $\spr{H}>\frac{3}{4}\spr{\cH}$. Assume that there is a second hypothesis $h_2$ that is also uncovered. However, by definition we know that there exists a test which distinguishes $h_1$ and $h_2$ so $h_2$ needs to belong to a response of size at most $\spr{\cH}/4$, a contradiction. 
...
Algorithm \ref{alg:worstDecisionTree} is recursive and a set cover $\cC$ is used to split the instance and make recursive calls.
...
\begin{lemma}\label{lemma:cover_sep_worst_case}
    ...
    Let $S^*$ be an optimal solution to $\ProblemPCSC$ on instance $\br{\cH, \xi(\cT), \preceq_{\xi}, f=1/4}$.
...
%Therefore there exists $t\in \sepcover{\cI}$ and a response $\cH'$ of size $\spr{\cH'}\leq \spr{\cH}/2$ such that hypotheses in $\cH'$ are not covered by $\sepcover{\cI}$, otherwise the claim holds trivially, since all hypotheses are covered. Let $\cH'\subseteq U_{t,j}$ (since $\cH'$ is a response to a test $t$, such $U_{t,j}$ always exists). By assumption, we have that $\spr{U_{t,j}-\cH'} < \spr{\cH}/4$. Therefore, we have that $\spr{U_{t,j}} = \spr{\cH'}+\spr{U_{t,j}-\cH'}<3/4\cdot \spr{\cH}$ which by definition means that $h$ is covered by $\sepcover{\cI}$, a contradiction.
        Therefore there exists first test $t\in \sepcover{\cI}$ and a response $U\in t$ that corresponds to a child $u$ of $t$ such that hypotheses in $U$ are not covered by $\xi(\sepcover{\cI})$ and $\spr{\cH_u}\leq\spr{\cH}/2$.
        ...
        We have that $U\setminus\cH_u$ is a subset of hypotheses covered by other tests in $\sepcover{\cI}$ and hence by assumption, $\spr{U\setminus\cH_u}< \spr{\cH}/4$.
        Therefore, we have that $\spr{U} = \spr{\cH_u}+\spr{U\setminus\cH_u}<3/4\cdot \spr{\cH}$ which means that hypothesis in $U$ are covered by $\sepcover{\cI}$, a contradiction.
    \end{proof}
...
$\cO\br{\frac{\alpha}{\log\br{\frac{4\gamma}{4\gamma -1}}} \cdot \log n}$-approximation algorithm for $\ProblemPCWCDT$. 
...
Throughout this subsection, we treat $\sepcover{\cI}$ as a sequence rather than a set, since the order of tests matters for computing the average-case cost. We also want that after removal of all tests in $\sepcover{\cI}$ from the decision tree, each subtree has at most half of the total probability mass of hypotheses.
...
\begin{lemma}\label{lemma:cover_sep_average_case}
    Let $\cI=\br{\cH, \cT, \preceq}$ be any $\ProblemPCACDT$ instance. Let $S^*$ be an optimal solution to $\ProblemPCMSSC$ on instance $\br{\cH, \xi(\cT), \preceq_{\xi}, f=1/4}$.
...
\begin{proof}
    ...
    Let $\sigma$ be the shortest prefix of $\sepcover{\cI}$ that covers at least $p\br{\cH}/4$ probability mass, $\sum_{u\in \cov\br{\sigma}}p\br{u}\geq p\br{\cH}/4$.
    ...
    \begin{itemize}
        \item $\sigma$ covers $h$. Consider the first test $t$ that sepcovered $h$ in $\sepcover{\cI}$. By definition, the tests prior to $t$ in $\sigma$ cover at most $p\br{\cH}/4$ probability mass. Since at the moment of sepcovering, $h$ belonged to a response of size at most $p\br{\cH}/2$, we know that $t$ also covers $h$ in $\sigma$. This means that the contribution of $h$ to $\coverageTime\br{\sigma}$ is at most its contribution to $\COSTA\br{\sepcover{\cI}}$.
...
\begin{theorem}\label{thm:PCMSSC-to-PCACDT}
    If there is an $\br{\beta, \mu}$-bicriteria approximation algorithm for $\ProblemPCMSSC$, then there is a $\cO\br{\frac{\beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log \br{m+n}}$-approximation algorithm for $\ProblemPCACDT$. 
...
\begin{proof}
    ...
    The base case when $s_z\leq1$ (equivalently, $\spr{\cH}=1$) is trivial since the cost of the decision tree is 0.
...
\begin{corollary}\label{cor:PCACDT-special-cases}
    If there is an $\br{\beta, \mu}$-bicriteria approximation algorithm for $\ProblemPCMSSC$, then there is a $\cO\br{\frac{\beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log \br{m+n}}$-approximation algorithm for $\ProblemPCACDT$. 
    % In particular when $\mu = \cO\br{1}$, the approximation is $\cO\br{\beta \cdot \log \br{m+n}}$. 
    This yields the following approximation ratios:
    \begin{itemize}
