...
\begin{observation}
    Let $I$ be any instance of $\ProblemPCWAL$. Then $\spr{P^*\br{I}} \leq \OPTW\br{I}$.
\end{observation}

We will use $\spr{P^*\br{I}}$ as a lower bound on $\OPTW\br{I}$ in the analysis of the approximation algorithm for $\ProblemPCWAL$. We have the following lemma:
\begin{lemma}\label{lemma:cover_sep_worst_case}
    Let $I=\br{\mathcal{H}, \mathcal{T}, \mathcal{F}}$ be any $\ProblemPCWAL$ instance. Let $S^*$ be the optimal solution for the PCSC on instance $\br{\mathcal{H}, \mathcal{T}, \mathcal{F}, f}$ where $f=1/4$ and a test $t$ covers $h\in \mathcal{H}$ if $\brc{U_{t}\br{u}}\leq \frac{3}{4}\cdot \brc{\mathcal{U}}$. Then, $\spr{S^*} \leq \spr{P^*\br{I}}$.
\end{lemma}
    \begin{proof}
...
$O\br{\frac{\alpha}{\log\br{\frac{2\gamma}{2\gamma -1}}} \cdot \log n}$-approximation algorithm for $\ProblemPCWAL$. In particular when $\gamma = O\br{1}$, the approximation is $O\br{\alpha \cdot \log n}$.
\end{theorem}
\begin{proof}
...
$$
\DD{w powyższym nie widać jakie znaczenie ma wybór $D$.}
