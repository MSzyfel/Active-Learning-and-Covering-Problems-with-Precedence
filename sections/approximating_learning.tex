...
Therefore, from now on we will assume that the input instance $\cI=\br{\cH, \cT, \cF, p}$ of $\ProblemPCACAL$ is such that for each $h\in\cH$, $p(h)=i/j$ for integers $i<j=O(nm)$, without impacting the asymptotic approximation ratio of our algorithm.
...
\begin{theorem}
    If there is an $\br{\beta, \mu}$-bicriteria approximation algorithm for $\ProblemPCMSSC$, then there is a $O\br{\frac{\beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log n}$-approximation algorithm for $\ProblemPCACAL$. In particular when $\mu = O\br{1}$, the approximation is $O\br{\beta \cdot \log n}$.
\end{theorem}
\begin{proof}
