...
\begin{observation} \label{obs:sepcoverWC}
    Let $\cI$ be any instance of $\ProblemPCWCAL$.
...
\begin{observation} \label{obs:dec-tree-averaging}
    If $D$ is a decision tree for a $\ProblemPCACAL$ instance with hypotheses $\cH$ with probability distribution $p$, then $\COSTA\br{D}=\sum_{v\in V(D)}p(v)\cdot\spr{\cH_v}$.
%    $$
...
\begin{observation} \label{obs:WCdecomposition}
    Let $D$ be any decision tree for an instance of $\ProblemPCACAL$ and let $S=\tests{D}{v}$ for any node $v$ of $D$.
    Then,
    $$
...
\begin{lemma} \label{lemma:rounding}
    Let $\cI=\br{\cH, \cT, \cF, p}$ be any instance of $\ProblemPCACAL$ and let $p'$ be the $q$-rounding of $p$.
...
\begin{lemma}\label{lemma:cover_sep_average_case}
    Let $\cI=\br{\cH, \cT, \cF}$ be any uniform distribution $\ProblemPCACAL$ instance. Let $S^*$ be an optimal solution to $\ProblemPCMSSC$ on instance $\br{\cH, \xi(\cT), \preceq_{\xi}, f}$ with $f=1/4$.
...
\begin{theorem}\label{thm:PCMSSC-to-PCACAL}
    If there is an $\br{\beta, \mu}$-bicriteria approximation algorithm for $\ProblemPCMSSC$, then there is a $O\br{\frac{\beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log n}$-approximation algorithm for the uniform distribution $\ProblemPCACAL$. In particular when $\mu = O\br{1}$, the approximation is $O\br{\beta \cdot \log n}$.
\end{theorem}
\begin{proof}
...
By combining the Lemma \ref{lemma:rounding} and Theorem \ref{thm:PCMSSC-to-PCACAL}, we immediately obtain the following corollary.
\begin{corollary}
    If there is an $\br{\beta, \mu}$-bicriteria approximation algorithm for $\ProblemPCMSSC$, then there is a $O\br{\frac{\beta}{\log\br{\frac{4\mu}{4\mu -1}}}\cdot \log \br{m+n}}$-approximation algorithm for $\ProblemPCACAL$. In particular when $\mu = O\br{1}$, the approximation is $O\br{\beta \cdot \log \br{m+n}}$.
\end{corollary}
