...
In this section we present approximation algorithms for the problems $\ProblemPCWCDT$ and $\ProblemPCACDT$ by reducing them to the problems $\ProblemPCSC$ and $\ProblemPCMSSC$, respectively. The overarching idea of these reductions is that we want to use the solution of the appropriate covering problems in order to find a precedence-closed sequence of tests, such that if one were to perform these tests in the decision tree, then the size of each response would be at most $c\cdot n$ for some constant $c<1$. Moreover, the cost of performing all the tests in this sequence should be upper bounded by $O\br{\OPT}$ (up to the approximation ratio of the covering problem). The rest of the decision tree is built recursively and since the size of each response is small, the depth of the recursion and as a consequence the loss in the approximation ratio is logarithmic in $n$. In what follows we make these ideas precise. We begin with some additional notation.
...
Let $\cH'\subseteq \cH$. By $t\br{\cH'}$ we denote the test $t$ restricted to hypotheses in $\cH'$, i.e., for each reply $H\in t$, we have a reply $H\cap \cH'$ in $t\br{\cH'}$. By $D\setminus V$ we denote the collection of subtrees (a forest) obtained by the removal of all nodes in $V$ from $D$.
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item $\sigma$ does not cover $h$. Since $h$ is sepcovered by some test $t$ in $\sepcover{\cI_{\cU}}$ but not covered by $\sigma$, the contribution of $h$ to $\coverageTime\br{\sigma}$ is $p\br{\sigma}$ and its contribution to $\COSTA\br{\sepcover{\cI_{\cU}}}$ is at least $p\br{\sigma}$.
    \end{itemize}
...
\end{observation}
\begin{proof}
    Assume that $\cH\setminus\cU$ is not empty, otherwise we actually have $\COSTA\br{D_{\cC}} \leq \alpha \cdot \COSTA\br{\sepcover{\cI_{\cU}}}$. Let $h\in \cH\setminus\cU$. Trivially, we have that the contribution of $h$ to $\COSTA\br{D_{\cC}}$ is at most $\spr{\cC}$. We also have that $\spr{\cC}\leq \coverageTime\br{\cC}$. Therefore we have that 
    \[
    \COSTA\br{D_{\cC}} \leq \alpha \cdot \COSTA\br{\sepcover{\cI_{\cU}}} + \spr{\cC} \leq 2\alpha \cdot \COSTA\br{\sepcover{\cI_{\cU}}}.
    \]
\end{proof}
...
\begin{align*}
    ...
    \leq & 2\alpha \cdot \COSTA\br{\sepcover{\cI_{\cU}}} + \sum_{\cH'(D')} \frac{2\alpha}{\log\br{\frac{5\beta}{5\beta -1}}}\cdot \log n' \cdot \OPT\br{\cI'} \\
