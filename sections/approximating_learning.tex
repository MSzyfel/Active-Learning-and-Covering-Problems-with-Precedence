...
\begin{definition}[Coversep]
    Let $D$ be any decision tree for the PCACAL problem instance $\mathcal{I}=\br{\mathcal{H}, \mathcal{T}, \mathcal{F}}$. We define a sequence of tests $C_D$ called \emph{coversep} as follows. Initially, $C_D$ is empty and $\mathcal{H}' = \mathcal{H}$. While $\spr{\mathcal{H}'}> \spr{\mathcal{H}}/2$, we append to $C_D$ the test $r\br{D_{\mathcal{H}'}}$ and update $\mathcal{H}'$ to be the set of hypotheses corresponding to the child of $D_{\mathcal{H}'}$ that contains the most hypotheses. If $\COST\br{D}=\OPT\br{\mathcal{I}}$, then we denote $C^*\br{\mathcal{I}} = C_D$ (ties broken arbitrarily).
\end{definition}
