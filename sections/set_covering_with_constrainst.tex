...
The key to solve PCSC and PCMSSC is to obtain appropriate approximations for $\ProblemMDPCS$ and $\ProblemBMDPCS$
%An approximation algorithm for $\ProblemMDPCS$ can be used as an essential subroutine in our algorithms for PCSC and PCMSSC.
By \cite{PCMSSC}, the following solution to $\ProblemMDPCS$ on instance $\br{\cG}$ provides a $O\br{\sqrt{m}}$-approximation:
$\cA = \argmax_{S\in\cG} \brc{\Delta\br{\closure{S}}}$.
...
An $\cA$ that satisfies
    $$
    \cA=\argmax_{S \in \mathcal{G}, \spr{\closure{S}}\leq B} \brc{\Delta\br{\closure{S}}}.
    $$
...
It should also be observed that if we would like to achieve an approximation algorithm for $\ProblemPCSC$ with an approximation ratio independent of $f$, we can simply utilize the $\br{O\br{\sqrt{m\log\br{n}}}, 1}$-approximatio ratio for $\ProblemPCSC$ presented in the previous section. To do so, one simply needs to find the minimal value of $B$ which suffices to cover $k$ elements using the $\br{O\br{\sqrt{m\log\br{n}}}, 1}$-approximation algorithm for $\ProblemBPCSC$. This can be done via binary search. The resulting approximation ratio will be $O\br{\sqrt{m\log\br{n}}}$. Note that however, since in our applications $1/f=O\br{1}$ this yields worse approximation ratio then the $\br{\sqrt{m}/f, 2}$-approximation algorithm presented in this section.
...
\begin{proof}
    ...
    \begin{lemma}\label{lem:sigma-coverage}
        We have that $\spr{\cov\br{\sigma_{l^*}, \cU}} \geq \frac{f\cdot n}{\beta}$ and $\spr{\sigma_{l}} = O\br{\alpha}\cdot \spr{\zeta^*}$.
    \end{lemma}
    \begin{proof}
        Since for the optimal cover $\zeta^*$, $\spr{\zeta^*}\leq a^{l^*}$ and covers at least $f\cdot n$ elements, it is a feasible solution to the $\ProblemBPCSC$ instance $\cI_{l^*+1}$. Therefore we know that $\spr{\sigma_{l}} = O\br{\alpha}\cdot \spr{\zeta^*}$ and $\spr{\cov\br{\sigma_{l^*}, \cU}} \geq \frac{f\cdot n}{\beta}$.
    \end{proof}
    \begin{lemma}\label{lem:combined-coverage-cost}
        We have that $\spr{\cov\br{\pi_{l^*} \circ \sigma_{l^*}, \cU}}\geq \frac{f\cdot n}{\beta}$ and $\COST\br{\pi_{l^*} \circ \sigma_{l^*}} = O\br{\alpha\cdot\beta}\cdot \COST\br{\zeta^*}$.
    \end{lemma}
    \begin{proof}
...
\subsection{Special cases of the precedence relationships} \label{subsection:SpecialCases}
\begin{theorem}
