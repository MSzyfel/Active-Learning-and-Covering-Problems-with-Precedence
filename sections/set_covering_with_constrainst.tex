...
\begin{theorem}
    If there exists a $(\alpha,\beta)$-approximation algorithm for $\ProblemPCSC$, then there exists an $\br{O\br{\alpha\cdot\beta}, \beta}$-approximation algorithm for $\ProblemPCMSSC$.
\end{theorem}
\begin{proof}
    Let $a=\frac{3\beta-1}{3\beta-2}<2$.
    Consider the procedure shown in Algorithm~\ref{alg:fPCMSSC}.
    \input{pseudocodes/f_PCMSSC.tex}
    % \MS{Analiza do przepisania z \url{https://arxiv.org/abs/1003.0722}, Sekcja 3.2. Uwaga!, trzeba sprawdzić jaka jest wartość $a$ w zależności od $alpha$, ale wydaje mi się że nadal 5/4 działa. Poniżej zrobiłem mały boilerplate analizy do uzupełnienia.}
    ...
    \begin{lemma}\label{lem:pi-spread-cost}
        We have that $\spr{\pi_{l^*}} = \Theta\br{\alpha }\cdot \spr{\zeta^*}$ and $\COST\br{\pi_{l^*}} = O\br{\alpha\cdot\beta}\cdot \COST\br{\zeta^*}$.
    \end{lemma}
    \begin{proof}
        ...
        by our choice of $a$. 
        \begin{enumerate}
            \item If $\beta=1$, then we have:
        \begin{align*}
            Q&\leq
            \alpha\cdot\COST\br{\zeta^*}\cdot\frac{\br{\frac{3\beta-1}{3\beta-2}}^4}{\br{\frac{3\beta-1}{3\beta-2}-1}^2}=O\br{\alpha}\cdot\COST\br{\zeta^*}
        \end{align*}
        \item Assume that $\beta\neq 1$. Then:
        \begin{align*}
            Q&\leq 
            \alpha\cdot\COST\br{\zeta^*}\cdot\frac{\br{\frac{3\beta-1}{3\beta-2}}^4}{\br{\frac{3\beta-1}{3\beta-2}-1}^2\cdot\br{1-\frac{\br{\beta-1/3}}{\beta}}}
            \\&=
            \alpha\cdot\COST\br{\zeta^*}\cdot\frac{3\beta\cdot \br{\frac{3\beta-1}{3\beta-2}}^4}{\br{\frac{3\beta-1}{3\beta-2}-1}^2}
            =O\br{\alpha\cdot\beta}\cdot\COST\br{\zeta^*}
        \end{align*}
