...
Hence by \eqref{eq:Ai} we get $\spr{\cA_l}\leq \frac{2n}{k}\spr{\cCopt}$.
This gives $\spr{\cC_l}=\sum_{i=1}^l\spr{\cA_i}\leq\frac{4n}{k}\cdot\spr{\cCopt} = \frac{4}{f}\cdot\spr{\cCopt}$.
...
\begin{theorem} \label{thm:MDPCStoPCSC}
    If there exists a $\gamma$-approximation algorithm for $\ProblemMDPCS$, then there exists a $\br{\frac{4\gamma}{f}, 2}$-bicriteria approximation algorithm for $\ProblemPCSC$.
\end{theorem}
