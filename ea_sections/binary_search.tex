...
\DD{troche nie widze tego lematu: co jesli precedensy to dwa lancuchy biegnace od srodka sciezki do jej konca/poczatku? Być może źle czytam, gdyz nie bylo zdefiniowane jak rozumiemy zbior krawedzi bedacy precedence closed?}
...
\begin{lemma}\label{ea:lemma:cutcost}
...
\begin{theorem}\label{ea:thm:binary-search-approximation}
    The above algorithm is an $\cO\br{\log n}$-approximation for the $\ProblemPCACDT$ in the case of binary search with precedence constraints.
\end{theorem}
\begin{proof}
    ...
    \end{align*}
    where the first inequality follows from the definition of average cost, the second inequality follows from the Lemma \ref{ea:lemma:cutcost}, inductive hypothesis, the fact that each subproblem has size at most $n/2$ and Lemma \ref{ea:lemma:subinstances_avg}. The claim follows.
\end{proof}
