\section{Max-Density Precedence-Closed Subfamily} \label{ea:sec:MDPCS}
...
%\input{ea_pseudocodes/mdpcs_greedy.tex}
...
Additionally, we show that a slightly modified greedy rule achieves a $\budget$-approximation for the parametrized version of the problem, $\ProblemBMDPCS$.
...
\begin{theorem}\label{ea:thm:BMDPCS-greedy}
...
\section{Set covering with precedence constraints} \label{ea:sec:SC}
...
\subsection{$\ProblemBPCSC$ via $\ProblemBMDPCS$} \label{ea:subsection:BPCSC}

The approximation algorithm is the greedy procedure shown in Algorithm~\ref{ea:alg:BPCSC}.
% For the analysis, let $\cC_i$ and $\cA_i$, $i\in\brc{1,\ldots,l}$, be the sets $\cC$ and $\cA$ obtained in the $i$-th iteration; $\cC_0=\emptyset$.
% Note that $\cov\br{\cA_i,\cU\setminus\cov\br{\cC_{i-1}}}$ is the set of element in $\cU$ that are being covered in the $i$-th iteration. 
%, and for each $u\in\cov\br{\cA_i,\cU\setminus\cC_{i-1}}$ denote $c(u):=\Delta\br{\cA_i,\cU\setminus\cC_{i-1}}$.
\input{ea_pseudocodes/B_PCSC}
The two following lemmas give Theorem~\ref{ea:thm:BPCSC}.
\begin{lemma}\label{ea:lem:BPCSC-cost}
    Let $\cC$ be the cover returned by Algorithm~\ref{ea:alg:BPCSC}. Then, $\spr{\cC} \leq \br{\sqrt{m\cdot H_n}+1}\cdot \budget$.
\end{lemma}
% \begin{proof}
...
%     \end{enumerate}
% \end{proof}
\begin{lemma}\label{ea:lem:BPCSC-coverage}
    Let $\cC$ be the cover returned by Algorithm~\ref{ea:alg:BPCSC}. Then, $\spr{\cov\br{\cC}} \geq \spr{\cov\br{\cCopt}}$.
\end{lemma}
% \begin{proof}
...
%     (Note that such $t$ exists since we are analyzing Algorithm~\ref{ea:alg:BPCSC} under the assumption that the loop works indefinitely, i.e., until all items are covered).
...
%     \begin{claim}\label{ea:lem:cost-per-element}
...
%     \end{proof}
%
%     Using Claim~\ref{ea:lem:cost-per-element}, the sum of prices of all elements in $\cC_{t-1}$ is bounded by $\sum_{j=1}^k c(u_j)\leq \budget^2H_k$.
...
% \end{proof}
%Lemmas \ref{ea:lem:BPCSC-cost} and \ref{ea:lem:BPCSC-coverage} prove the following.
\begin{theorem} \label{ea:thm:BPCSC}
...
\subsection{$\ProblemPCSC$ via $\ProblemMDPCS$}\label{ea:subsection:PCSC}
Let $k=f\cdot n$.
In order to prove the main theorem of this section consider a greedy procedure shown as Algorithm~\ref{ea:alg:PCSC}.
We note a subtle fact of finishing the loop once $k/2$ elements are covered and not iterating till reaching $k$.
This is because the last set $\cA$ may be of unbounded size but it can be argued that the size of an optimal solution to the instance from the last iteration is lower-bounded by $\Omega(k)$, which is enough to obtain the required approximation ratio.
\input{ea_pseudocodes/set_cover.tex}
\begin{theorem} \label{ea:thm:MDPCStoPCSC}
...
\end{theorem}
% \begin{proof}
% Denote by $\cA_i$ the set $\cA$ from the $i$th iteration of Algorithm~\ref{ea:alg:PCSC}, $i\in\brc{1,\ldots,l}$.
...
\begin{corollary}\label{ea:cor:BPCSC-to-PCSC}
...
\subsection{$\ProblemPCMSSC$ via $\ProblemBPCSC$} \label{ea:subsection:PCMSSC}
...
Consider the procedure shown in Algorithm~\ref{ea:alg:fPCMSSC}.
% Denote by $\cCopt$ any optimal solution to $\ProblemPCMSSC$ on the input $(\cU, \cS, \preceq, f)$.
Let $l^*$ be such that $a^{l^*-1} < \spr{\cCopt} \leq a^{l^*}$.
\input{ea_pseudocodes/f_PCMSSC.tex}
We start with a lemma that relates the costs and the lengths of $\pi_{l^*}$ and $\cCopt$.
    \begin{lemma}\label{ea:lem:pi-spread-cost}
        We have that $\spr{\pi_{l^*}} = \Theta\br{\alpha \cdot \beta}\cdot \spr{\cCopt}$ and $\COST\br{\pi_{l^*}} = \bigo\br{\alpha\cdot\beta^3}\cdot \COST\br{\cCopt}$.
    \end{lemma}
    % \begin{proof}
    ...
    %     \begin{equation} \label{ea:eq:pi}
    %     \spr{\pi_{l^*}} \leq \alpha\cdot \sum_{i=1}^{l^*} a^{i+1}\leq \frac{\alpha\cdot a^{l^*+2}}{a-1}\leq \frac{\alpha\cdot a^{2}\spr{\cCopt}}{a-1}\leq4\alpha\cdot \br{3\beta-1}\cdot\spr{\cCopt}
    %     \end{equation}
    %     Since in the extension step we ensure that $\spr{\pi_{l^*}} \geq \alpha\cdot \beta\cdot a^{l^*}$, we have that $\spr{\pi_{l^*}} = \Theta\br{\alpha \cdot \beta}\cdot \spr{\cCopt}$ and the first part of the lemma follows.

    %     For every $i\in [l^*]$, denote $n_i^*=\spr{\cov\br{\prefix{\cCopt}{a^{i}}}}$ and note that $n_{l^*}^* = \spr{\cov\br{\cCopt}}$. Similarly, for every $i\in \brc{1,\dots,l^*}$ let $n_i=\spr{\cov\br{\tau_{l^*}^1\circ\dots\circ \tau_{l^*}^i}}$. Additionally, define $n_0=n_0^*=0$. We have:
    %     \begin{align*}
    ...
    %     \end{align*}
    %     The last inequality is due to \eqref{eq:pi} and the bound on $\spr{\cCopt}$.
    %     Moreover, we have that:
    %     \begin{align*}
    %     2\cdot\COST\br{\cCopt}&\geq a^{l^*-1}\br{n-n_{l^*-1}^*}+ \sum_{i=1}^{l^*-1}a^{i-1}\br{n_i^*-n_{i-1}^*}+\br{n-n_{l^*}^*}\cdot\spr{\cCopt}
    %     \\&\geq
    ...
    %     Consider any iteration $i\in \brc{1,\dots,l^*}$ of the inner for loop. Let $U_i=\cU\setminus \cov\br{\prefix{\cCopt}{a^{i-1}}}$ be the set of uncovered elements at the beginning of iteration $i$ which is provided as a part of the $\cI_i$ instance to the $\ProblemBPCSC$ algorithm. We have that $\OPT\br{\cI_i}\geq n_i^*-n_{i-1}$ since $\prefix{\cCopt}{a^{i}}$ is a feasible solution to instance $\cI_i$ of $\ProblemBPCSC$ such that $\spr{\cov\br{\prefix{\cCopt}{a^{i}}, U_i}}\geq n_i^*-n_{i-1}$.
    ...
    %         \\&\leq
    %         \frac{2\cdot a^4}{a-1}\cdot\COST\br{\cCopt}+\frac{\beta-1}{\beta}\cdot\sum_{i=1}^{l^*}a^{i+3}\cdot \br{n-n_{i-1}}
    %         \\&=
    %         \frac{2\cdot a^4}{a-1}\cdot\COST\br{\cCopt}+\frac{\br{\beta-1}\cdot a}{\beta}\cdot\sum_{i=0}^{l^*-1}a^{i+3}\cdot \br{n-n_{i}}
    %         \\&\leq
    %         \frac{2\cdot a^4}{a-1}\cdot\COST\br{\cCopt}+\frac{\br{\beta-1}\cdot a}{\beta}\cdot\br{a-1}\cdot\frac{Q}{\alpha}.
    %     \end{align*}
    %     By rearranging the above inequality we obtain:
    %     \begin{align*}
    %         Q&\leq \alpha\cdot\COST\br{\cCopt}\cdot\frac{2\cdot a^4}{\br{a-1}^2\cdot\br{1-\frac{\br{\beta-1}\cdot a}{\beta}}}
    %         \\&=
    %         \alpha\cdot\COST\br{\cCopt}\cdot\frac{2\cdot\br{\frac{3\beta-1}{3\beta-2}}^4}{\br{\frac{3\beta-1}{3\beta-2}-1}^2\cdot\br{1-\frac{\br{\beta-1}\cdot \br{3\beta-1}}{\beta\cdot\br{3\beta-2}}}}
    %         \\&=
    %         \alpha\cdot\COST\br{\cCopt}\cdot\frac{2\cdot \br{3\beta-1}^2\cdot \br{\frac{3\beta-1}{3\beta-2}}^4}{\br{1-\frac{\br{\beta-1}\cdot \br{3\beta-1}}{\beta\cdot\br{3\beta-2}}}}
    %     \end{align*}
    ...
    %     \begin{align*}
    %         Q&\leq
    %         \alpha\cdot\COST\br{\cCopt}\cdot\frac{\br{3\beta-1}^2\cdot\br{\frac{2\cdot 3\beta-1}{3\beta-2}}^4}{\br{1-\frac{\br{\beta-1/3}}{\beta}}}
    %         \\&=
    %         \alpha\cdot\COST\br{\cCopt}\cdot 6\beta\cdot \br{3\beta-1}^2\cdot \br{\frac{3\beta-1}{3\beta-2}}^4
    %         \leq 864\cdot\alpha\cdot\beta^3\cdot\COST\br{\cCopt}
    %     \end{align*}
    %     The lemma follows.
    % \end{proof}
    ...
    \begin{lemma}\label{ea:lem:sigma-coverage}
        We have that $\spr{\cov\br{\sigma_{l^*}}} \geq \frac{f\cdot n}{\beta}$ and $\spr{\sigma_{l^*}} = O\br{\alpha}\cdot \spr{\cCopt}$.
    \end{lemma}
    % \begin{proof}
    %     Since the optimal cover $\cCopt$ satisfies $\spr{\cCopt}\leq a^{l^*}$ and covers at least $f\cdot n$ elements, it is a feasible solution to the $\ProblemBPCSC$ instance $\cI'=(U,\cS,\preceq,a^{l^*})$ for which the $(\alpha,\beta)$-approximation $\sigma_{l^*}$ has been computed after the inner loop.
    %     Therefore we know that $\spr{\sigma_{l^*}} = O\br{\alpha}\cdot \spr{\cCopt}$ and $\spr{\cov\br{\sigma_{l^*}}} \geq \frac{f\cdot n}{\beta}$.
    % \end{proof}
    \begin{lemma}\label{ea:lem:combined-coverage-cost}
        We have that $\spr{\cov\br{\pi_{l^*} \circ \sigma_{l^*}}}\geq \frac{f\cdot n}{\beta}$ and $\COST\br{\pi_{l^*} \circ \sigma_{l^*}} = O\br{\alpha\cdot\beta^3}\cdot \COST\br{\cCopt}$.
    \end{lemma}
    % \begin{proof}
    %     Since $\pi_{l^*} \circ \sigma_{l^*}$ covers all of the elements covered by $\sigma_{l^*}$, we have by Lemma~\ref{ea:lem:sigma-coverage} that $\spr{\cov\br{\pi_{l^*} \circ \sigma_{l^*}}}\geq \frac{f\cdot n}{\beta}$.
    ...
    %     Lemma \ref{ea:lem:pi-spread-cost} implies that $\COST\br{\pi_{l^*}}=\sum_{u\in\cU} t_u = O\br{\alpha\cdot\beta^3}\cdot \COST\br{\cCopt}$.
    %     Observe, that for each $u\in\cov\br{\pi_{l^*}}$, its cover time in $\pi_{l^*} \circ \sigma_{l^*}$ is also $t_u$.
    %     For each $u\notin\cov\br{\pi_{l^*}}$, by Lemma~\ref{ea:lem:pi-spread-cost} its cover time in $\pi_{l^*}$ is $t_u=\Omega\br{ \alpha\cdot\beta\cdot a^{l^*}}=\Omega\br{\alpha\cdot a^{l^*}}$ and its cover time in $\pi_{l^*} \circ \sigma_{l^*}$ is at most $\spr{\pi_{l^*} \circ \sigma_{l^*}}=\spr{\pi_{l^*}} + \spr{\sigma_{l^*}} = \bigo\br{\alpha\cdot\beta}\cdot \spr{\cCopt} + \bigo\br{\alpha}\cdot a^{l^*} = \bigo\br{\alpha\cdot\beta}\cdot \spr{\cCopt}$.
    %     Therefore, its cover time in $\pi_{l^*} \circ \sigma_{l^*}$ is at most a constant factor larger than its cover time in $\pi_{l^*}$.
    %     As a consequence, due to Lemma~\ref{ea:lem:pi-spread-cost} we have that $\COST\br{\pi_{l^*} \circ \sigma_{l^*}} = O\br{\alpha\cdot\beta^3}\cdot \COST\br{\cCopt}$.
    % \end{proof}
Lemma \ref{ea:lem:combined-coverage-cost} directly implies the following theorem.
\begin{theorem}\label{ea:thm:BPCSC-to-PCMSSC}
...
\end{theorem}
% It should be noted that in all cases when we use Theorem~\ref{ea:thm:BPCSC-to-PCMSSC} we will have $\beta=\bigo\br{1}$, thus yielding a $\br{\bigo\br{\alpha}, \bigo\br{1}}$-approximation algorithm for $\ProblemPCMSSC$.

\subsection{Special cases of precedence constraints} \label{ea:subsection:SpecialCases}
\begin{theorem}\label{ea:thm:BPCSC-inforest}
...
\end{corollary}
\begin{theorem}\label{ea:thm:BPCSC-outforest}
...
%     We create the metric as follows: we introduce a root node $r$ and for each set $S\in\cS$ we add a representative node $v_S$ connected to its every succesor in the outforest with an edge of length $1$.
...
%     Moreover, since the tour starts at the root, we have that if a representative of a set $W$ is visited, it the representative of all of its predecessors in the outforest have been visited.
%     Therefore, any feasible solution to $\ProblemGSO$ on this metric corresponds to a feasible solution to $\ProblemBPCSC$ on the original instance with the same cost and coverage, and vice versa. Thus, by Corollary \ref{ea:cor:BPCSC-to-PCSC} and Theorem \ref{ea:thm:BPCSC-to-PCMSSC} the theorem follows.
%     \DD{Uwaga: Thm. \ref{ea:thm:BPCSC-to-PCMSSC} nie przenosi ratio, do sprawdzenia.}
% \end{proof}
...
% \begin{theorem}\label{ea:thm:MDPCS-shallow-ancestry}
...
% \begin{proof}
%  Let $\cA^*$ be an optimal solution and let $\cA$ be a greedy solution (cf. Section~\ref{ea:sec:MDPCS}).
