...
\medskip
The idea behind the algorithms for the worst case (Section~\ref{ea:subsection:PCWCDT}) and the average case (Section~\ref{ea:subsection:PCACDT}) is similar.
...
We also note that the sequence of tests $t_1,t_2\ldots$ in Algorithm~\ref{ea:alg:worstDecisionTree} is a `substitute' of sepcover (cf. Lemma~\ref{ea:lemma:cover_sep_worst_case}).
...
Let $D_{\cC}$ be a decision tree restricted only to tests from $\cC$. By Lemmas \ref{ea:lemma:subspace_opt} and \ref{ea:lemma:cover_sep_worst_case} we have:
    \begin{observation} \label{ea:obs:DS}
        Consider $D_{\cC}$ computed in Algorithm~\ref{ea:alg:worstDecisionTree}, that uses a $\br{\gamma,\alpha}$-approximation algorithm for $\ProblemPCSC$ as the $\blackBox$ subroutine.
...
If there is a polynomial-time bicriteria $\br{\alpha, \beta}$-approximation algorithm for $\ProblemPCSC$, then there is an
...
% \end{observation}
The analysis of the average case cost takes similar steps as for the worst case cost and leads to the following final theorem.
