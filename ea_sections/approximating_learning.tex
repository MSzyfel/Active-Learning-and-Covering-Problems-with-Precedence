\section{Decision Trees via Covering Problems} \label{ea:sec:AL}
...
We begin with few additional symbols. For any node $v$ of the $D$ by $\cH_v$ we will mean the set of hypotheses such that the test $t$ associated with $v$ is performed in $D$ if the set of possible hypotheses is $\cH_v$. Let $\cH'\subseteq \cH$. By $t\br{\cH}$ we denote the test $t$ restricted to hypotheses in $\cH'$, i.e., for each response $H\in t$, we have a response $H\cap \cH'$ in $t\br{\cH'}$. Finally, by $D\setminus V$ denote the collection of subtrees (a forest) obtained by the removal of all nodes in $V$ from $D$.
We will use the following immediate monotonicity properties. 
\begin{lemma}\label{ea:lemma:subspace_opt}
...
\begin{definition}[Sepcover]
  ...
  Then the set $\tests{D}{v}$ is called a \emph{sepcover} for $D$ and is denoted by $\cS\br{D}$.
  If $\COSTW\br{D}=\OPT\br{\cI}$, then we write $\sepcover{\cI} = \cS\br{D}$ (ties broken arbitrarily).
\end{definition}
% Figure \ref{ea:fig:sepcover} illustrates the definition of sepcover. 
...
By extension, for any subset $S\subseteq\cT$, $\xi(S)=\brc{\xi(t) \mid t\in S}$. We also set $\cU=\bigcup_{t\in\cT}\xi(t)$. 
\begin{lemma}
    We have $\spr{\cU}\geq \frac{4}{5}\cdot\spr{\cH}$.
\end{lemma}
% \begin{proof}
%     If $\spr{\cH}\leq4$, then there exists a test $t$ which separates some pair of hypotheses, and therefore $\xi\br{t}=\cH$. Assume that $\spr{\cH}\geq4$. We actually show, that there is at most one hypothesis that is not in $\cU$. To see this observe that if a hypothesis $h_1\notin \cU$, then for every test $t\in\cT$, it holds that for the response $H\in t$ containing $h_1$, $\spr{H}>\frac{3}{4}\cdot\spr{\cH}$. Assume that there is a second hypothesis $h_2\notin \cU$. By definition we know that there exists a test which distinguishes $h_1$ and $h_2$, so $h_2$ needs to belong to a response of size at most $\frac{3}{4}\cdot\spr{\cH}$, a contradiction. Thus $\spr{\cU}\geq \min_{i\geq5}\brc{\frac{i-1}{i}}\cdot\spr{\cH}=\frac{4}{5}\cdot\spr{\cH}$.
% \end{proof}
...
% \begin{figure}[h]
% \centering
% \input{ea_figures/sepcover.tex}
% \caption{Sepcover sequence in a decision tree}
% \label{ea:fig:sepcover}
% \end{figure}

\medskip
The idea behind the algorithms for the worst case (Section~\ref{ea:subsection:PCWCDT}) and average case (Section~\ref{ea:subsection:PCACDT}) is similar.
Hence we extract here the common generic subroutine (see Algorithm~\ref{ea:alg:worstDecisionTree}) used in both cases.
The only difference is a black-box procedure that is used to solve a particular subproblem -- either $\ProblemPCSC$ or $\ProblemPCMSSC$ respectively.
Let $\cC$ be the cover obtained for the instance $\br{\cU, \xi\br{\cT}, \preceq_{\xi}, f=1/4}$ by the black-box procedure on the corresponding instance. Assume that $\cC$ contains some ordering of its elements. If $\cC$ is the output of an approximation algorithm for $\ProblemPCSC$, then this ordering is a topological ordering of the tests in $\cC$ according to the precedence constraints.
Note that the $\cC$ is supposed to play the role of $\sepcover{\cI}$ in our lower bounding scheme.
%  according to the lower bounds in Lemmas~\ref{lemma:cover_sep_worst_case} and~\ref{lemma:cover_sep_average_case} given in the subsections below.
We iteratively build a decision tree $D$ by appending tests associated with consecutive sets from $\cC$. During each such iteration, we look at all possible responses to the appended test and for each sufficiently small response $\cH'$ ($\spr{\cH'} \leq \frac{5\beta-1}{5\beta}\cdot \spr{\cH}$), we recursively call \ProcDecisionTree on the subinstance induced by $\cH'$. If there exists a large response $\cH'$ ($\spr{\cH'} > \frac{5\beta-1}{5\beta}\cdot \spr{\cH}$), then we do not make a recursive call for it and simply continue appending tests from $\cC$ as a response associated with $\cH'$. Note that since $\frac{5\beta-1}{5\beta}>\frac{1}{2}$ for any $\beta \geq 1$, there is at most one such response. 
% Hence we indeed can afford constructing an arbitrary decision tree $D_S$ on the tests corresponding to the cover in $S$.
\input{ea_pseudocodes/worst_case_learning.tex}


\subsection{Precedence Constrained Worst Case Decision Tree} \label{ea:subsection:PCWCDT}

We will use sepcovers to lowerbound the cost of optimal solution:
\begin{observation} \label{ea:obs:sepcoverWC}
...
\begin{lemma}\label{ea:lemma:cover_sep_worst_case}
...
%         Therefore there exists first test $t\in \sepcover{\cI}$ and a response $H\in t$ that corresponds to a child $u$ of $t$ such that hypotheses in $H$ are not covered by $\xi(\sepcover{\cI})$ and $\spr{\cU_u}\leq\spr{\cU}/2$.
...
%We now state an algorithm (Algorithm~\ref{ea:alg:worstDecisionTree}) that outputs a decision tree for an input instance $\cI=\br{\cH, \cT, \preceq}$ of $\ProblemPCWCAL$.

%     Algorithm \ref{ea:alg:worstDecisionTree} is recursive and works as follows.
...
Let $D_{\cC}$ be a decision tree restricted only to tests from $\cC$. The following observation is due to Lemmas \ref{ea:lemma:subspace_opt} and \ref{ea:lemma:cover_sep_worst_case}.:
    \begin{observation} \label{ea:obs:DS}
        Consider $D_{\cC}$ computed in Algorithm~\ref{ea:alg:worstDecisionTree}, that uses an $\br{\gamma,\alpha}$-approximation algorithm for $\ProblemPCSC$ as the $\blackBox$ subroutine.
...
Using Lemmas \ref{ea:lemma:subspace_opt}, \ref{ea:lemma:subinstances_avg}, \ref{ea:lemma:cover_sep_worst_case} and Observations \ref{ea:obs:sepcoverWC} and \ref{ea:obs:DS}, we can prove the following theorem.:
\begin{theorem} \label{ea:thm:PCSC-to-PCWCDT}
If there is an $\br{\alpha, \beta}$-bicriteria approximation algorithm for $\ProblemPCSC$, then there is an
...
%         \end{align*}
%         where the second inequality follows by the induction hypothesis and Observation~\ref{ea:obs:DS}, the third inequality follows by Observation~\ref{ea:obs:sepcoverWC}, Lemma \ref{ea:lemma:subspace_opt} and the fact that $n'=\spr{\cH'} \leq \br{1-\frac{4}{5}\cdot\frac{1}{4\cdot\beta}}\cdot n=\frac{(5\beta -1)}{5\beta}\cdot n$ for every response $\cH'$ associated with a test outside of $D_{\cC}$.
%         This concludes the proof of the theorem.
% \end{proof}

Therefore by Theorem \ref{ea:thm:PCSC-to-PCWCDT} we obtain the following corollary:
\begin{corollary}\label{ea:cor:PCWCDT-special-cases}
    There exist polynomial-time algorithms for $\ProblemPCWCDT$ with the following guarantees:
    \begin{itemize}
        \item $\cO\br{\log n}$-approximation for inforests, by Theorem~\ref{ea:thm:BPCSC-inforest}, and Corollary~\ref{ea:cor:BPCSC-to-PCSC}.
        \item $\cO\br{\log^2n}$-approximation for outforests, by and Theorem~\ref{ea:thm:BPCSC-outforest}.
        \item General $\cO\br{\sqrt{m}\cdot\log n}$-approximation, by Theorem~\ref{ea:thm:MDPCStoPCSC}, and Theorem~\ref{ea:thm:MDPCStoPCSC}.
    \end{itemize}
\end{corollary}



\subsection{Precedence Constrained Average Case Decision Tree} \label{ea:subsection:PCACDT}
...
% \begin{observation} \label{ea:obs:dec-tree-averaging}
...
% Observation~\ref{ea:obs:dec-tree-averaging} allows us to decompose the cost criterion as follows.
% \begin{observation} \label{ea:obs:WCdecomposition}
...
% \begin{observation} \label{ea:obs:sepcoverAC}
...
% % \begin{lemma} \label{ea:lemma:rounding}
...
% %     By Observation~\ref{ea:obs:dec-tree-averaging}, $\COSTA\br{D, \cI'}=\sum_{v\in V(D)}p'(v)\cdot\spr{\cH_v}$.
...
% %     \end{align*}
% %     where the second inequality is by Observation~\ref{ea:obs:dec-tree-averaging} and $\spr{\cH_v}\leq n$ for each node $v$, the third inequality is by $s_z\geq nm$ and the last inequality follows since $\COSTA\br{D, \cI} \geq \sum_{h\in\cH}p\br{h} = 1$.
...
% We have the following lemma, analogous to Lemma \ref{ea:lemma:cover_sep_worst_case}.
% \begin{lemma}\label{ea:lemma:cover_sep_average_case}
...
% %     Such a subsequence exists, by repeating the argument used in the proof of Lemma \ref{ea:lemma:cover_sep_worst_case}.
% %     Take an arbitrary $h \in \cH$.
% %     There are two cases to consider:
% %     \begin{itemize}
% %         \item $\sigma$ covers $h$. Consider the first test $t$ that sepcovered $h$ in $\sepcover{\cI}$. By definition, the tests prior to $t$ in $\sigma$ cover at most $\spr{\cU}/4$ items. Since at the moment of sepcovering, $h$ belonged to a response of size at most $\spr{\cU}/2$, we know that $t$ also covers $h$ in $\sigma$. This means that the contribution of $h$ to $\coverageTime\br{\sigma}$ is at most its contribution to $\COSTA\br{\sepcover{\cI}}$.
...
% The following observation is analogous to Observation~\ref{ea:obs:DS}:
% \begin{observation} \label{ea:obs:DS-average}
%     Consider $D_{\cC}$ computed in Algorithm~\ref{ea:alg:worstDecisionTree}, that uses an $\br{\alpha,\beta}$-approximation algorithm for $\ProblemPCMSSC$ as the $\blackBox$ subroutine.
%     Then, $\COSTA\br{D_{\cC}} \leq \alpha \cdot \COSTA\br{\sepcover{\cI}}$.
% \end{observation}
The analysis of the algorithm for the average case cost is similar to the worst case cost. We have:
\begin{theorem}\label{ea:thm:PCMSSC-to-PCACDT}
    If there is an $\br{\alpha, \beta}$-bicriteria approximation algorithm for $\ProblemPCMSSC$, then there is a $\cO\br{\frac{\alpha}{\log\br{\frac{5\beta}{5\beta -1}}}\cdot \log n}$-approximation algorithm for $\ProblemPCACDT$. 
...
%     Let $D$ be the decision tree returned by this call.
%     In order to prove the theorem it is enough to argue that
%     \begin{equation} \label{ea:eq:D-average}
...
%     = & \frac{\alpha}{\log\br{\frac{5\beta}{5\beta -1}}}\cdot \log n \cdot \OPT\br{\cI},
% \end{align*}
% where the first inequality is by Observation~\ref{ea:obs:WCdecomposition}, the second inequality follows by the induction hypothesis and Observation~\ref{ea:obs:DS-average}, the third inequality follows by Observation~\ref{ea:obs:sepcoverAC}, Lemma~\ref{ea:lemma:subspace_opt} and the fact that $n'=\spr{\cH'} \leq \br{1-\frac{4}{5}\cdot\frac{1}{4\cdot\beta}}\cdot n=\frac{(5\beta -1)}{5\beta}\cdot n$ for every response $\cH'$ associated with a test outside of $D_{\cC}$.
% This concludes the proof of the theorem.
% \end{proof}

Therefore by Theorem \ref{ea:thm:PCMSSC-to-PCACDT} we obtain the following corollary:

\begin{corollary}\label{ea:cor:PCACDT-special-cases}
    There exist poly-time algorithms for $\ProblemPCACDT$ with the following guarantees:
    \begin{itemize}
        \item $\cO\br{\log n}$-approximation for inforests, by Corollary~\ref{ea:cor:BPCSC-inforest-to-PCMSSC}.
        \item $\cO\br{\log^2 n}$-approximation for outforests, by Theorem~\ref{ea:thm:BPCSC-outforest}.
        \item $\cO\br{\sqrt{m}\cdot\log^{3/2} n}$-approximation for general precedence constraints, by Theorem~\ref{ea:thm:BPCSC-to-PCMSSC} and Theorem~\ref{ea:thm:BPCSC}.
    \end{itemize}
\end{corollary}
