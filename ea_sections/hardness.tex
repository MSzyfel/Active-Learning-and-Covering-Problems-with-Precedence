\section{Hardness} \label{ea:sec:hardness}
\newcommand{\coord}[3]{h[#1, #2, #3]}
\newcommand{\sepedge}[1]{t^{sep}[#1]}
\newcommand{\varedge}[1]{t^{var}[#1]}
\newcommand{\litedge}[1]{t^{cla}[#1]}

In this section, we establish hardness and inapproximability results for the precedence-constrained covering and decision tree problems introduced in previous sections. Throughout all reductions involving fractional set cover ($\ProblemPCSC$), we assume that $f = 1$, meaning that each set can be selected at most once.
...
\begin{theorem}\label{ea:thm:PCWCDT-NP-hard}
...
% 	\begin{figure}[htbp]
% 		\centering
% 		\input{ea_figures/clause_gadget.tex}
% 		\caption{Clause gadget for $c_i = \{v_i^1, v_i^2, v_i^3\}$.}
% 		\label{ea:fig:clause_gadget}
% 	\end{figure}
%
% 	\begin{figure}[htbp]
% 		\centering
% 		\input{ea_figures/variable_gadget.tex}
% 		\caption{Variable gadget for $v_i$.}
% 		\label{ea:fig:variable_gadget}
% 	\end{figure}
%
% 	\begin{figure}[htbp]
% 		\centering
% 		%\input{ea_figures/reduction_example.tex}
...
% 		\label{ea:fig:reduction_path}
% 	\end{figure}
%
% 	\begin{figure}[htbp]
% 		\centering
% 		\input{ea_figures/reduction_precedence.tex}
...
% 		\label{ea:fig:reduction_precedence}
% 	\end{figure}
...
\begin{theorem}\label{ea:thm:PCSC-to-PCWCDT-hardness}
...
\end{proof}
\begin{theorem}\label{ea:thm:PCMSSC-to-PCACDT-hardness}
...
\end{proof}
% \begin{theorem}\label{ea:thm:PCSC-to-PCACDT-hardness}
...
\begin{theorem}\label{ea:thm:PCSC-outforest-hardness}
    $\ProblemPCSC$ with outforest precedence constraints cannot be approximate within a factor of $\cO\br{\log^{2-\epsilon} n}$ for any $\epsilon > 0$ unless $\text{NP}\subseteq \text{ZTIME}\br{n^{\text{polylog}(n)}}$.
\end{theorem}
\begin{proof}
    To show this we prove that $\ProblemGST$ on tree metrics is reducible to $\ProblemPCSC$ with outforest precedence constraints. 

    Since $\ProblemGST$ on trees cannot be approximated within a factor of $\cO\br{\log^2 n}$ unless $\text{NP}\subseteq \text{ZTIME}\br{n^{\text{polylog}(n)}}$ \cite{PolylogarithmicInapproximability}. Note that in the latter reduction all of the distances are of form $2^{-h}$ with exponent ranging from $0$ to $h=\cO\br{\log^{1-\epsilon} n}$. Therefore, we can scale all of them by $2^h=\text{poly}\br{n}$ to obtain integer weights without affecting the approximation ratio.
...
\end{proof}
\begin{theorem}\label{ea:thm:PCMSSC-outforest-hardness}
...
\begin{proof}
    ...
    Since $\ProblemLPGST$ on trees cannot be approximated within a factor of $\cO\br{\log^2 n}$ unless $\text{NP}\subseteq \text{ZTIME}\br{n^{\text{polylog}(n)}}$ \cite{ApproxAlgsForOptDTsAndAdapTSPProblems}. 
    Note that in the above problem some group weights might be $0$. To alleviate this difficuilty we scale up all of the weights up by a factor of $n^2$ and if a given weight is $0$ we set it to $1$. Observe that this scaling changes the optimal value by at most a factor of $2$ and thus does not affect the asymptotical hardness of approximation.
...
\end{proof}
As an immediate corollaries of Theorem \ref{ea:thm:PCSC-to-PCWCDT-hardness} and \ref{ea:thm:PCMSSC-to-PCACDT-hardness} we obtain the following results:
\begin{corollary}\label{ea:thm:PCWCDT-outforest-hardness}
...
\begin{corollary}\label{ea:thm:PCACDT-outforest-hardness}
...
\begin{conjecture}[Planted Dense Subgraph (PDS) Conjecture] \label{ea:problem:PDS}
...
\begin{theorem}\label{ea:thm:PCMSSC-PDS-hardness}
...
Using the reduction from $\ProblemPCMSSC$ to $\ProblemPCACDT$ from the previous section we immediately obtain the following results:
\begin{corollary}\label{ea:thm:PCACDT-PDS-hardness}
...
Below we show that a similar reduction can also be used to obtain the same inapproximability result for $\ProblemPCSC$. The construction and analysis is mostly the same as in \cite{PCMSSC} but we include it here for completeness. In the reduction we will often make use of the Chernoff Bound which is as follows:
\begin{theorem}[Chernoff Bound]\label{ea:thm:chernoff}
...
\begin{theorem}\label{ea:thm:PCSC-PDS-hardness}
...
\begin{proof}
    ...
    Fix an arbitrary element $j \in \{1,\dots, n\}$. We first bound the number of vertices in $\mathcal{P}$ whose auxiliary sets contain $j$. Since $\mathbb{E}[\spr{\{v\colon j\in U_v, v \in V(\mathcal{P})\}}] = p\sqrt{N}$, by Chernoff's bound (Theorem \ref{ea:thm:chernoff}) with $\delta = 1/2$, we have:
    \[
    ...
    \[
    \mathbb{P}\left[\left|\{(u,v)\colon j\in U_u, j\in U_v, u,v \in V(\mathcal{P}), u < v\}\right| \leq \frac{p^2\cdot N}{16}\right] \leq N^{-4\cdot N^{1/8+\gamma/4}}.
    \]
...
\begin{corollary}\label{ea:thm:PCWCDT-PDS-hardness}
