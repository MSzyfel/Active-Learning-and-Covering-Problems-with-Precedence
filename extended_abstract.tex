...
The latter is brought by a problem of finding a maximum density precedence closed subfamily, where the density is defined as the ratio of the number of items the family covers to its size. By doing so we provide $\cO^*(\sqrt{m})$-approximation algorithms for all of the aforementioned problems.
...
\begin{keywords}%
  \textsc{Optimal Decision Trees}, \textsc{Set Cover}, Precedence Constraints, Approximation Algorithms%
\end{keywords}

\input{ea_sections/introduction}
\input{ea_sections/preliminaries}
% \input{sections/binary_search}
\input{ea_sections/approximating_learning}
\input{ea_sections/set_covering_with_constrainst}
